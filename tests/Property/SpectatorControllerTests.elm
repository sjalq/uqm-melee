    ...
    describe "Watching matches and assigning pilots"
        [ test "spectators receive state without occupying a player seat" <|
            \() ->
                let
                    ( host, deliveries ) =
                        Room.handle "watcher" "w" (Room.WatchRoom code) initial
                in
        ...
        , test "spectators cannot pause, change pilots, ready up or submit controls" <|
            \() ->
                let
                    watching =
                        initial |> send "watcher" "w" (Room.WatchRoom code)
                in
        ...
        , test "spectator disconnect removes the audience membership" <|
            \() -> initial |> send "watcher" "w" (Room.WatchRoom code) |> Room.disconnect "watcher" "w" |> Tuple.first |> get |> Maybe.map (.spectators >> Dict.size) |> Expect.equal (Just 0)
        ...
        , test "computer versus human works with the human in the top seat" <|
            \() ->
                let
                    host =
                        initial |> send "host" "h" (Room.SetController Bottom Room.Computer) |> send "guest" "g" (Room.JoinRoom code) |> send "guest" "g" Room.Ready
                in
                case get host |> Maybe.map (.game >> .phase) of
        ...
        , test "a player cannot replace another connected human with a computer" <|
            \() ->
                let
                    full =
                        initial |> send "guest" "g" (Room.JoinRoom code)
                in
        ...
        , test "the backend advances AI battles and sends spectator snapshots" <|
            \() ->
                let
                    launched =
                        initial |> send "host" "h" (Room.SetController Bottom Room.Computer) |> send "host" "h" (Room.SetController Top Room.Computer) |> send "host" "h" Room.Ready |> send "watcher" "w" (Room.WatchRoom code)
